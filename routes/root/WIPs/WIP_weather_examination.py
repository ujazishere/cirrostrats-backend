...
class WIPWeatherExamination_archive:
    
    def __init__(self):
        """"""
        """ # Datis metar and taf loaded. init loads file names from the pickles folder."""
        self.base_path = r"C:\Users\ujasv\OneDrive\Desktop\pickles"
        self.directory_file_names = os.listdir(self.base_path)            # get all file names in pickles folder
        self.weather_bulk = {}
        # self.datis_bulk = self.load_em_all('datis_info_stack')
    

    def load_em_all(self, file_name_contains:str):
        """ ## Use either 'bulk_metar' or 'bulk_taf' or 'datis' for `file_name_contains` to load them all."""
        # get just the ones with datis_info_stack in name
        file_names = [i for i in self.directory_file_names if file_name_contains in i]

        weather_bulk = []
        for i in file_names:
            path = self.base_path+"\\"+ i
            with open(path, 'rb') as f:
                weather_bulk += pickle.load(f)
        print(f'total {file_name_contains} files: {len(file_names)}', f'total {file_name_contains}: {len(weather_bulk)}')
        self.weather_bulk[file_name_contains] = weather_bulk
        return weather_bulk


    def zulu_anomaly(self, weather_type:str):
        """ Find datis, or metar that has zulu time anomaly. """
        print('Metar takes a while to load- fetches from data lake. Please wait... possibly 20+ seconds')
        if weather_type == 'datis':
            weather = self.load_em_all('datis')
            zulu_pat = '[0-9]{4}Z'
        elif weather_type == 'metar':
            weather = self.load_em_all('bulk_metar')
            zulu_pat = '[0-9]{6}Z'
        else:
            raise ValueError("weather must be 'datis' or 'metar'")

        # check type weather can only be `datis` or `metar`
        for i in weather:

            a = re.findall(zulu_pat, i)
            if len(a) > 1:
                ind = i.index(a[1])
                print(a, i[ind-13:ind+15])
                print(i)

    def code_lake(self):
        """ Pick and chose what you need from here."""
        import re
        import torch
        import pickle
